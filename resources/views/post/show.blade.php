                ...
                @if ($post->user_id === auth()->user()->id)
                    <form action="{{ route('post.destroy', $post) }}" method="POST">
                        ...
                        <input type="submit" value="Eliminar Publicacion"
                            class="bg-red-500 hover:bg-red-600 transition-colors cursor-pointer uppercase font-bold  p-3 text-white">
                    </form>
                @endif
            @endauth
            ...
            <div class="shadow bg-white p-5 mb-5">
                ...
                @auth
                    ...
                    <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST"
                        ...
                        @csrf
                        <div class="mb-5">
                            <label for="comentario" class="mb-2 block uppercase text-gray-800 font-bold">
                                Añade un comentario
                            </label>
                            <textarea class="border p-3 w-full rounded-lg @error('comentario') border-red-500 @enderror" id="comentario"
                                name="comentario" placeholder="Agrega un nuevo Comentarios  "></textarea>
                            @error('comentario')
                ...
                <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario)
                            <div class="p-5 border-gray-300 border-t">
